...
struct EcosystemCommandCenter {
    func execute(_ command: EcosystemCommand, on state: inout EcosystemState) {
        switch command {
    ...
    func applyRealtimeTick(on state: inout EcosystemState, at date: Date) {
    ...
    private func applyScene(_ scene: SmartScene, on state: inout EcosystemState) {
        switch scene {
        case .relax:
            for index in state.devices.indices {
